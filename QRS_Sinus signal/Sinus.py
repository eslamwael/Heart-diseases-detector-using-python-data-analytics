...
while (i< (len(peak_index) -1)):
    ...
    print(RR_interval)
    if(RR_interval >330): # 330 is the average of the maximum RR interval
        missing_beat_value.append(peak_value[i])
        missing_beat_index.append(peak_index[i] +200) # 200 is the average RR interval
    ms_dist = ((RR_interval / 256) * 1000.0) #Convert sample distances to ms distances
    RR.append(ms_dist)
    i= i+1

print(missing_beat_index)
print(missing_beat_value)
# Plotting

plt.subplot(211)
plt.plot(Signal_final, color='Red')
...
plt.scatter(missing_beat_index, missing_beat_value, color='Black', marker='*')
plt.xlim(0, 2000)
plt.show()